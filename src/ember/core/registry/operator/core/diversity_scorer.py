...
from ember.core.utils.eval.base_evaluator import IEvaluator
from ember.core.utils.eval.diversity_evaluators import DiversityEnsembledEvaluator
...
class DiversityScoringOperator(
    ...
    def __init__(self, *, embedding_model: EmbeddingProviderModel, evaluator: IEvaluator) -> None:
    ...
    def forward(
        self, 
        *, 
        inputs: DiversityScoringOperatorInputs
        ...
        score = self.evaluator(embedding_model=self.embedding_model).evaluate(inputs.responses).score
        # logger instead
