...
class DiversityEnsembledEvaluator(IEvaluator[List[str], None]):
    ...
    def __init__(self, embedding_model: EmbeddingProviderModel):
        ...
        if not self.embedding_model:
            LOGGER.warning("DiversityEnsembledEvaluator isn't initialized with an embedding model")
    ...
    def evaluate(
        ...
        **kwargs
        ...
        if not system_output:
            LOGGER.debug("DiversityEnsembledEvaluator didn't receive an output")
            return EvaluationResult(is_correct=False, score=-1)
        if self.embedding_model is None:
            LOGGER.debug("DiversityEnsembledEvaluator wasn't initialized with an embedding model")
            return EvaluationResult(is_correct=False, score=-1)
        if len(system_output) == 1:
            LOGGER.debug("DiversityEnsembledEvaluator only received one string of text")
...
class DiversityCosineSimilarityEvaluator(IEvaluator[List[str], None]):
    ...
    def __init__(self, embedding_model: EmbeddingProviderModel = None):
        self.embedding_model = embedding_model
        if not self.embedding_model:
            LOGGER.warning("DiversityCosineEvaluator isn't initialized with an embedding model " + 
    ...
    def evaluate(
        ...
        **kwargs
        ...
        if not system_output:
            LOGGER.debug("DiversityCosineEvaluator didn't receive an output")
            return EvaluationResult(is_correct=False, score=-1)
        if self.embedding_model is None:
            LOGGER.debug("DiversityCosineEvaluator wasn't initialized with an embedding model")
            return EvaluationResult(is_correct=False, score=-1)
        if len(system_output) == 1:
            LOGGER.deubg("DiversityCosineEvaluator only received one string of text")
            return EvaluationResult(is_correct=True, score=0)
...
class DiversityEditDistanceEvaluator(IEvaluator[List[str], None]):
...
class DiversityNoveltyEvaluator(IEvaluator[List[str], None]):
    ...
    def __init__(self, embedding_model: EmbeddingProviderModel = None):
        self.embedding_model = embedding_model
        if self.embedding_model is None:
            LOGGER.warning("DiversityNoveltyEvaluator isn't initialized with an embedding model " + 
                            "Using default OpenAI embedding model instead")
            self.embedding_model = create_openai_embedding_model()
    ...
    def evaluate(self,
        ...
        if len(system_output) == 0:
            LOGGER.warning("Length of inputs to evaluate function is zero")
            return EvaluationResult(is_correct=False, score=-1, metadata={})
